...
class WufooApiWrapper extends WufooApiWrapperBase
{
     ...
     *
     * Remember, the URL changes with the form title, so it's best to use the hash.
     *
     * @param  string $formIdentifier can be the url or hash.
    ...
    /**
     * Gets all fields for a given form or report by url or hash.
     *
     * Remember, the URL changes with the form/report title, so it's best to use the hash.
     *
    ...
    public function getFields($formIdentifier, $from = 'forms')
    {
        ...
        $fieldHelper = new ValueObject\WufooFieldCollection();
    ...
    /**
     * Gets all entries from a given form or report by url or hash.
     *
     * Remember, the URL changes with the form/report title, so it's best to use the hash.
     *
    ...
    /**
     * Gets entry count for a given form or report by url or hash.
     *
     * Remember, the URL changes with the form/report title, so it's best to use the hash.
     *
     * @param  string $identifier a URL or Hash
    ...
    */
     ...
     *
     * Remember, the URL changes with the report title, so it's best to use the hash.
     *
     * @param  string $reportIdentifier can be the url or hash.
     ...
     * Remember, the URL changes with the report title, so it's best to use the hash.
     *
     * @param  string string $reportIdentifier can be the url or hash.
    ...
    public function webHookPut($formIdentifier, $webHookUrl, $handshakeKey, $metadata = false)
    {
        ...
        return new Response\WebHookResponse($result->WebHookPutResult->Hash);
    }
    ...
    public function webHookDelete($formIdentifier, $hash)
    {
        ...
        return new Response\WebHookResponse($result->WebHookDeleteResult->Hash);
    }
}
